...
public final class Autos {
    ...
    public static CommandBase exampleAuto(SwerveSubsystem swerve) {
        ...
        if (onTheFly) {
            // Simple path with holonomic rotation. Stationary start/end. Max velocity of 4 m/s and
            ...
            // This is just an example event map. It would be better to have a constant, global
            // event map
            ...
            // not every time you want
            // to create an auto command. A good place to put this is in RobotContainer along with
            // your subsystems.
            SwerveAutoBuilder autoBuilder =
                    new SwerveAutoBuilder(
                            ...
                            new PIDConstants(
                                    Autonomous.yAutoPID.p,
                                    Autonomous.yAutoPID.i,
                                    Autonomous.yAutoPID.d),
                            // PID constants to correct for translation error (used to create the X
                            // and Y PID controllers)
                            new PIDConstants(
                                    Autonomous.angleAutoPID.p,
                                    Autonomous.angleAutoPID.i,
                                    Autonomous.angleAutoPID.d),
                            // PID constants to correct for rotation error (used to create the
                            // rotation controller)
                            swerve::setChassisSpeeds,
                            // Module states consumer used to output to the drive subsystem
                            eventMap,
                            false,
                            // Should the path be automatically mirrored depending on alliance
        ...
        PathPlannerTrajectory path =
                PathPlanner.generatePath(
                        ...
                        PathPoint.fromCurrentHolonomicState(
                                swerve.getPose(), swerve.getRobotVelocity()),
                        new PathPoint(
                                aprilTagField
                                        .getTagPose(id)
                                        .get()
